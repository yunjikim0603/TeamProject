...
<script type="text/javascript">
// 게시글 수정
function modify_article(article_num) {
	if (confirm("게시글을 수정하시겠습니까 ? ") == true) {
		location.href="CommunityModifyForm.any?num=${article.num }";
	} else {
		return false;
	}
}
...
// 게시글 삭제
function delete_article(article_num) {
	if (confirm("게시글을 삭제하시겠습니까 ? ") == true) {
		location.href="CommunityDeletePro.any?num=${article.num }";
	} else {
		return false;
	}
}
...
function delete_comment(comment_num){
	if (confirm("댓글 삭제하시겠습니까?") == true){    
		location.href="CommentDeletePro.anyC?post_num=${article.num }&nickname=${sessionScope.nickname }&comment_num="+comment_num;
	}else{  
		return false;
	}
}
...
function modify_comment(comment_num){
	if (confirm("댓글 수정하시겠습니까?") == true){  
		location.href="CommentModifyForm.anyC?post_num=${article.num }&comment_num="+comment_num;
	}else{   
		return false;
	}
}
...
$(document).ready(function() {
	getComment(1);
		...
		function paging(nowPage){
			$.ajax({		
				url: "CommentPagingDetail.anyC?nowPage="+nowPage, // 요청 url
	            type: "POST", // post 방식
	            data: {
		...
		<div class="gtco-container">
			...
			<div class="w3-article">
				<div class="left-box">
					...
					<div class="w3-border w3-padding">
						첨부파일: <a href="CommunityFileDown.any?file_name=${article.file }" target="blank">${article.file }</a>
					</div>
					<br>
