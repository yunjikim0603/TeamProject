package academy_community.svc;

import static academy_community.db.JdbcUtil.close;
import static academy_community.db.JdbcUtil.commit;
import static academy_community.db.JdbcUtil.getConnection;
import static academy_community.db.JdbcUtil.rollback;

import java.sql.Connection;

import academy_community.dao.AcademyCommentDAO;
import academy_community.vo.AcademyCommentBean;

public class AcademyCommentWriteProService {

	public boolean registComment(AcademyCommentBean commentBean) {
//		System.out.println("CommentWriteProService!");
		
//		1. Connection 객체 가져오기
//		Connection con = JdbcUtil.getConnection(); // static 메서드 호출 시
		Connection con = getConnection(); // static import 를 사용했을 경우 클래스명 없이 static 메서드 호출
		
		// 2. DAO 객체 가져오기(싱글톤 패턴)
		AcademyCommentDAO commentDAO = AcademyCommentDAO.getInstance();
		
		// 3. DAO 객체에 Connection 객체 전달하기
		commentDAO.setConnection(con);
		
		// 4. DB 작업을 위한 DAO 객체의 메서드 호출
		int insertCount = commentDAO.insertComment(commentBean);
		
		boolean isCommentWrite = false;
		
		// insertCount 가 0보다 크면 성공, 아니면 실패
				if(insertCount > 0) {
					// 글쓰기 성공 시 commit 작업 수행 및 isWriteSuccess 변수값을 true 로 변경
					commit(con);
					
					// 글쓰기 성공 후 참조글번호(board_re_ref) 업데이트를 위한 작업 수행
//					boardDAO.updateBoard_re_ref(boardBean);
//					commit(con);
					
					isCommentWrite = true;
				} else {
					// 글쓰기 실패 시 rollback 작업 수행
					rollback(con);
				}
				
				// 5. Connection 객체 반환하기
				close(con);
				
				
				// 6. 글쓰기 성공 여부 리턴
				return isCommentWrite;
	}

	
}
